...
Private Sub GenerarArchivoIndividual(ByVal clienteNombre As String)
    ...
    ' ── 6. Form: confirmar carpeta, versión y nombre ─────────────────────────────────
    Dim msgInfo As String
    msgInfo = "Carpeta: " & rutaCarpeta & vbCrLf & vbCrLf & _
              msgVersion & vbCrLf & _
              "Versión propuesta: " & versionTag

    With FrmNombreArchivo
        .Inicializar rutaCarpeta, nombreSugerido, msgInfo
        .Show vbModal
        If .Cancelado Then
            Debug.Print "[CANCELADO] El usuario canceló el nombre de fichero."
            Unload FrmNombreArchivo
            Exit Sub
        End If
        ' Recoger nombre y carpeta (puede haber cambiado con el explorador)
        rutaCarpeta  = .CarpetaElegida
        nombreEditado = LimpiarExtension(Trim(.NombreElegido))
        Unload FrmNombreArchivo
    End With
...
Private Sub DesprotegerHoja(ByVal ws As Worksheet, ByVal pwd As String)
    ...
    ' DisplayAlerts=False evita que Excel muestre el diálogo de contraseña
    Application.DisplayAlerts = False
    On Error Resume Next

    ' Intento 1: sin contraseña (hojas protegidas sin clave)
    ws.Unprotect

    ' Intento 2: con contraseña ADP (hojas protegidas con clave)
