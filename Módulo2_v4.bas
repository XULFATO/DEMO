...
Private Const RUTA_POR_DEFECTO As String = "O:\ADP_SP\Clientes_Bob_LOPD\J18_ANCERT\J18_ANCERT\PRUEBAS\"
...
Private Sub GenerarArchivoIndividual(ByVal clienteNombre As String)
    ...
    rutaCarpeta = ResolverCarpetaDestino()
    If rutaCarpeta = "" Then
...
Private Function ResolverCarpetaDestino() As String
    Dim ruta As String

    If UnidadRedConectada(UNIDAD_RED) Then
        ' Red disponible: usar ruta fija, sin preguntar
        ruta = RUTA_POR_DEFECTO
        Debug.Print "  [RED] Usando ruta por defecto: " & ruta
    Else
