...
Private Function VerificarPassword(ByVal pwd As String) As Boolean
    ...
    ' Buscar la primera hoja que esté protegida
    For Each ws In ThisWorkbook.Worksheets
        If ws.ProtectContents Then
            ' Intentar desproteger sin contraseña primero
            ...
            If ws.ProtectContents Then
                ' Sigue protegida → tiene contraseña; probar con pwd
                Application.DisplayAlerts = False
                On Error Resume Next
                ws.Unprotect pwd
                On Error GoTo 0
                ...
                If ws.ProtectContents Then
                    ' La contraseña no funcionó → volver a proteger no hace falta,
                    ' la hoja ya sigue protegida. Informar fallo.
                    Debug.Print "  [ERROR PASS] La contraseña no abre: " & ws.Name
                    VerificarPassword = False
                    Exit Function
                Else
                    ' Contraseña correcta → volver a proteger como estaba
                    ws.Protect pwd
                ...
                End If
            End If
            ' Si llegamos aquí la hoja no tenía contraseña, seguir buscando
        End If
    Next ws

    ' No se encontró ninguna hoja protegida con clave → no hay nada que verificar
    Debug.Print "  [INFO PASS] No hay hojas protegidas con contraseña en el libro."
    VerificarPassword = True
End Function
