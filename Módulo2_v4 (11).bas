...
Private Sub GenerarArchivoIndividual(ByVal clienteNombre As String)
    ...
    ' ── 3. Determinar carpeta destino por defecto (red o local) ──────────────────────
    rutaCarpeta = ResolverCarpetaDestino(clienteNombre)
    If rutaCarpeta = "" Then
        Debug.Print "[CANCELADO] Sin carpeta destino."
        Exit Sub
    End If
    Debug.Print "[OK] Carpeta por defecto: " & rutaCarpeta

    ' ── 4. InputBox para confirmar o cambiar la carpeta destino ──────────────────────
    Dim rutaEditada As String
    rutaEditada = InputBox( _
        "Revise o modifique la carpeta de destino:" & vbCrLf & vbCrLf & _
        "(Pulse Aceptar para usar la ruta mostrada)", _
        "Carpeta de destino — " & clienteNombre, _
        rutaCarpeta)

    If Trim(rutaEditada) = "" Then
        Debug.Print "[CANCELADO] El usuario canceló la selección de carpeta."
        Exit Sub
    End If
    rutaCarpeta = Trim(rutaEditada)
    If Right(rutaCarpeta, 1) <> "\" Then rutaCarpeta = rutaCarpeta & "\"
    Debug.Print "[OK] Carpeta destino confirmada: " & rutaCarpeta

    ' ── 5. Calcular versión mirando los ficheros existentes en ESA carpeta ────────────
    Dim patronBase    As String
    Dim versionTag    As String
    Dim versionPrevia As Integer
    ...
    versionPrevia = VersionExistenteMaxima(rutaCarpeta, patronBase)
    versionTag    = "V" & Format(versionPrevia + 1, "00")
    msgVersion    = IIf(versionPrevia = 0, _
                        "No se encontró ninguna versión previa en la carpeta.", _
                        "Versión encontrada en carpeta: V" & Format(versionPrevia, "00"))
    nombreSugerido = patronBase & versionTag
    Debug.Print "[OK] Versión existente: " & IIf(versionPrevia = 0, "ninguna", "V" & Format(versionPrevia, "00")) & _
                " | Propuesta: " & versionTag

    ' ── 6. InputBox: confirmar carpeta, versión y nombre ─────────────────────────────
    msgVersion = IIf(versionPrevia = 0, _
                     "No hay versión previa en la carpeta.", _
                     "Versión encontrada: V" & Format(versionPrevia, "00"))

    nombreEditado = InputBox( _
        "Carpeta: " & rutaCarpeta & vbCrLf & vbCrLf & _
        msgVersion & vbCrLf & _
        "Versión propuesta: " & versionTag & vbCrLf & vbCrLf & _
        "Confirme o edite el nombre del fichero (sin extensión):", _
        "Archivo de salida — " & clienteNombre, _
        nombreSugerido)

    If Trim(nombreEditado) = "" Then
        Debug.Print "[CANCELADO] El usuario canceló el nombre de fichero."
        Exit Sub
    End If
    ...
    ' Preguntar si abrir la carpeta destino en el explorador
    If MsgBox("¿Desea abrir la carpeta de destino?", _
              vbQuestion + vbYesNo, "Abrir carpeta") = vbYes Then
        Shell "explorer.exe """ & ExtraerCarpeta(nombreFinal) & """", vbNormalFocus
    End If
    Exit Sub
...
' ======================================================================================
' VERSIÓN EXISTENTE MÁXIMA
' Devuelve el número de la versión más alta encontrada en la carpeta (0 si ninguna).
' El llamador suma 1 para obtener la versión propuesta.
' ======================================================================================

Private Function VersionExistenteMaxima(ByVal rutaCarpeta As String, _
                                         ByVal patronBase  As String) As Integer
    ...
    VersionExistenteMaxima = maxVersion
End Function
...
' Busca la primera hoja del libro que esté protegida con contraseña y comprueba
' que PASSWORD_HOJAS la abre correctamente.
...
Private Function VerificarPasswordEnCopia(ByVal wb As Workbook, _
    ...
    ' DisplayAlerts ya es False globalmente — ningún diálogo puede aparecer
    For Each ws In wb.Worksheets
        If ws.ProtectContents Then
            ...
            If ws.ProtectContents Then
                On Error Resume Next
                ws.Unprotect pwd   ' Intento con contraseña ADP
                On Error GoTo 0
                ...
                If ws.ProtectContents Then
                    ...
                    Exit Function
                Else
                    Debug.Print "  [OK PASS] Contraseña correcta en: " & ws.Name
...
Private Sub DesprotegerHoja(ByVal ws As Worksheet, ByVal pwd As String)
    ...
    ' Pasar siempre la contraseña directamente.
    ' El intento sin contraseña sobre una hoja con clave es lo que dispara
    ' el diálogo nativo de Excel — evitarlo completamente.
    On Error Resume Next
    ws.Unprotect pwd
    On Error GoTo 0

    ' Si sigue protegida puede que no tuviera contraseña (o sea distinta)
    If ws.ProtectContents Then
        On Error Resume Next
        ws.Unprotect
        On Error GoTo 0
...
Private Sub DesprotegerTodasHojas(ByVal wb As Workbook, ByVal pwd As String)
    Dim ws As Worksheet
    Debug.Print "[PASS] Desprotegiendo todas las hojas de: " & wb.Name
    For Each ws In wb.Worksheets
        DesprotegerHoja ws, pwd
    Next ws
End Sub

' Vuelve a proteger todas las hojas que quedaron sin protección (solo las que tenían clave)
Private Sub ReprotegerTodasHojas(ByVal wb As Workbook, ByVal pwd As String)
    Dim ws As Worksheet
    Debug.Print "[PASS] Reprotegiendo hojas del original..."
    For Each ws In wb.Worksheets
        If Not ws.ProtectContents Then
            On Error Resume Next
            ws.Protect Password:=pwd, DrawingObjects:=True, Contents:=True, Scenarios:=True
            On Error GoTo 0
            Debug.Print "    [OK] Reprotegida: " & ws.Name
        End If
    Next ws
End Sub
...
Private Sub EjecutarGeneracion(ByVal idConfig As String, ByVal indiceCol As Long, _
    ...
    ' Las hojas se desprotegen ANTES de copiar para que el temporal abra sin diálogos,
    ' y se vuelven a proteger en el original justo después.
    Debug.Print "[3] Desprotegiendo original para copia limpia..."
    DesprotegerTodasHojas ThisWorkbook, PASSWORD_HOJAS

    Debug.Print "[3b] Creando copia temporal..."
    ThisWorkbook.SaveCopyAs fTemporal

    Debug.Print "[3c] Reprotegiendo hojas del original..."
    ReprotegerTodasHojas ThisWorkbook, PASSWORD_HOJAS
    Debug.Print "  [OK] Temporal creado, original restaurado"

    ' Abrir temporal — sin hojas protegidas, sin diálogo de contraseña
    ...
    Debug.Print "[4] Temporal abierto sin diálogos"

    Debug.Print "[5] Desprotegiendo hojas de la copia (por si quedara alguna)..."
